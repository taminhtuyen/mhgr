<div id="chat-interface" class="d-none">
    ...
    <div class="chat-layout h-100 d-flex overflow-hidden">
            ...
            <div class="chat-list flex-grow-1 overflow-auto custom-scrollbar" style="position: relative;">
                <div class="tab-content" id="chatTabContent">
                    ...
                    <div class="tab-pane fade show active" id="buyer-pane" role="tabpanel">
                        @for($i = 1; $i <= 5; $i++)
                        <div class="chat-item p-3 d-flex align-items-center cursor-pointer position-relative {{ $i == 1 ? 'active' : '' }}" onclick="openChat('buyer_{{ $i }}')">
                            ...
                            <div class="dropdown chat-actions-btn"><button class="btn btn-icon no-arrow" type="button" data-bs-toggle="dropdown" data-bs-container="body" onclick="event.stopPropagation()"><i class="fa-solid fa-ellipsis"></i></button><ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 glass-effect"><li><a class="dropdown-item small" href="#"><i class="fa-solid fa-bell-slash me-2 text-muted"></i>Tắt thông báo</a></li><li><a class="dropdown-item small" href="#"><i class="fa-solid fa-bell me-2 text-success"></i>Bật thông báo</a></li><li><hr class="dropdown-divider"></li><li><a class="dropdown-item small text-danger" href="#"><i class="fa-solid fa-trash me-2"></i>Xóa hội thoại</a></li></ul></div>
                        </div>
                        @endfor
                    </div>
                    {{-- TAB 2: SELLER --}}
                    <div class="tab-pane fade" id="seller-pane" role="tabpanel">
                        @for($k = 1; $k <= 3; $k++)
                        <div class="chat-item p-3 d-flex align-items-center cursor-pointer position-relative" onclick="openChat('seller_{{ $k }}')">
                            ...
                            <div class="dropdown chat-actions-btn"><button class="btn btn-icon no-arrow" type="button" data-bs-toggle="dropdown" data-bs-container="body" onclick="event.stopPropagation()"><i class="fa-solid fa-ellipsis"></i></button><ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 glass-effect"><li><a class="dropdown-item small" href="#"><i class="fa-solid fa-bell-slash me-2 text-muted"></i>Tắt thông báo</a></li><li><hr class="dropdown-divider"></li><li><a class="dropdown-item small text-danger" href="#"><i class="fa-solid fa-trash me-2"></i>Xóa hội thoại</a></li></ul></div>
                        </div>
                        @endfor
                    </div>
                    {{-- TAB 3: SYSTEM --}}
                    <div class="tab-pane fade" id="system-pane" role="tabpanel">
                        <div class="chat-item p-3 d-flex align-items-center cursor-pointer position-relative" style="border-bottom: 1px solid var(--popup-border); background-color: rgba(var(--primary), 0.03);" onclick="openChat('system_main')">
                        ...
                        <div class="chat-item p-3 d-flex align-items-center cursor-pointer position-relative" onclick="openChat('staff_{{ $s }}')">
                            <div class="flex-shrink-0 position-relative"><img src="https://ui-avatars.com/api/?name=Staff+{{ $s }}&background=64748b&color=fff" class="rounded-circle me-2" width="45" height="45"><span class="position-absolute bottom-0 end-0 p-1 bg-success border border-light rounded-circle" style="margin-bottom: 2px; margin-right: 8px;"></span></div>
                            <div class="flex-grow-1 overflow-hidden pe-4"><div class="d-flex justify-content-between align-items-center"><h6 class="mb-0 text-truncate" style="color: var(--text-color); font-size: 0.9rem; font-weight: 600;">Nhân Viên {{ $s }}</h6><small class="text-muted" style="font-size: 0.7rem;">08:00</small></div><p class="mb-0 text-truncate small mt-1" style="color: var(--text-muted);">Đã gửi báo cáo ngày...</p></div>
                            <div class="dropdown chat-actions-btn"><button class="btn btn-icon no-arrow" type="button" data-bs-toggle="dropdown" data-bs-container="body" onclick="event.stopPropagation()"><i class="fa-solid fa-ellipsis"></i></button><ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 glass-effect"><li><a class="dropdown-item small" href="#"><i class="fa-solid fa-bell-slash me-2 text-muted"></i>Tắt thông báo</a></li><li><a class="dropdown-item small" href="#"><i class="fa-solid fa-bell me-2 text-success"></i>Bật thông báo</a></li><li><hr class="dropdown-divider"></li><li><a class="dropdown-item small text-danger" href="#"><i class="fa-solid fa-trash me-2"></i>Xóa hội thoại</a></li></ul></div>
                        </div>
                        @endfor
                    </div>
        ...
        <div class="chat-window flex-grow-1 d-flex flex-column h-100" id="chat-content-col">
            <div class="chat-header p-3 border-bottom d-flex align-items-center flex-shrink-0" style="background: var(--popup-bg); backdrop-filter: blur(5px); border-color: var(--popup-border) !important; z-index: 5;">
                ...
                <img src="https://ui-avatars.com/api/?name=Buyer+1&background=random" class="rounded-circle me-2" width="35" height="35">
                <div><div class="fw-bold lh-1" style="color: var(--text-color); font-size: 0.95rem;">Khách Mua 1</div><small class="text-success desktop-nav-element" style="font-size: 0.75rem;"><i class="fa-solid fa-circle" style="font-size: 8px;"></i> Online</small></div>
                <div class="ms-auto d-flex align-items-center gap-1">
                    <button class="btn btn-sm btn-icon text-primary"><i class="fa-solid fa-phone"></i></button>
                    <button class="btn btn-sm btn-icon text-primary"><i class="fa-solid fa-video"></i></button>
                    <button class="btn btn-sm btn-icon text-primary"><i class="fa-solid fa-circle-info"></i></button>
                    <button class="btn btn-sm btn-icon text-danger ms-1"><i class="fa-solid fa-triangle-exclamation"></i></button>
                </div>
            </div>
            <div class="chat-messages flex-grow-1 p-3 overflow-auto custom-scrollbar bg-transparent">
                @for($j = 1; $j <= 3; $j++)
                <div class="message-row d-flex mb-3"><img src="https://ui-avatars.com/api/?name=User&background=random" class="rounded-circle me-2 mt-1" width="30" height="30"><div class="message-wrapper" style="max-width: 75%;"><div class="message-bubble p-2 px-3 rounded shadow-sm" style="background-color: var(--link-hover-bg); color: var(--text-color); border: 1px solid transparent;">Xin chào shop, đơn hàng #{{ $j }}992 bao giờ thì giao ạ?</div><div class="msg-meta ms-1">10:0{{ $j }} AM</div></div></div>
                <div class="message-row d-flex mb-3 flex-row-reverse"><div class="message-wrapper text-end" style="max-width: 75%;"><div class="message-bubble p-2 px-3 rounded shadow-sm text-start" style="background-color: var(--link-hover-bg); color: var(--text-color); border: 1.5px solid var(--primary);">Chào bạn, bên mình đang đóng gói rồi ạ.</div><div class="msg-meta me-1">10:0{{ $j + 1 }} AM &bull; @if($j == 3)<span class="text-primary"><i class="fa-solid fa-check-double"></i> Đã xem</span>@elseif($j == 2)<span><i class="fa-solid fa-check-double"></i> Đã nhận</span>@else<span><i class="fa-solid fa-check"></i> Đã gửi</span>@endif</div></div></div>
                @endfor
            </div>
            <div class="chat-input-area p-3 border-top flex-shrink-0" style="background: var(--popup-bg); border-color: var(--popup-border) !important; z-index: 5;">
                <div class="input-group">
                    <button class="btn btn-outline-secondary border-end-0 text-primary" style="background-color: var(--bg-body); border-color: var(--popup-border);"><i class="fa-solid fa-plus"></i></button>
                    <input type="text" class="form-control border-start-0 border-end-0 chat-search-input" placeholder="Nhập tin nhắn..." style="background-color: var(--bg-body); color: var(--text-color); border-color: var(--popup-border);">
                    <button class="btn btn-primary px-3"><i class="fa-solid fa-paper-plane"></i></button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.openChat = function(id) {
            ...
            console.log("Admin opening chat:", id);
        }
