...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(localStorage.getItem('client_theme_preference') === 'dark') { themeToggle.checked = true; updateThemeText(true); }
        ...
        let currentNeonState = localStorage.getItem('client_neon_mode') || 1;
        ...
        updateViewStatus(localStorage.getItem('client_menu_view') || 'list');
        viewBtn.addEventListener('click', () => {
            const current = localStorage.getItem('client_menu_view') || 'list';
        ...
        document.getElementById('settings-reset-menu')?.addEventListener('click', () => {
            if(confirm('Đặt lại vị trí menu?')) {
                Object.keys(localStorage).forEach(key => { if(key.startsWith('client_menu_order_')) localStorage.removeItem(key); });
